...
import {
  parseStakeholderUpdate,
  mergeStakeholderUpdate,
  generateStakeholderMarkdown,
} from '@/lib/stakeholderParser';
import { StakeholderModal, StakeholderPanel } from '@/components/Stakeholder';
...
function StakeholderModeContent() {
  return <StakeholderModeInner />;
}

export default function StakeholderMode() {
  return (
    <Suspense fallback={<StakeholderModeLoading />}>
      <StakeholderModeContent />
    </Suspense>
  );
}

function StakeholderModeLoading() {
  return (
...
function StakeholderModeInner() {
  ...
  const handleSendMessageDirect = async (content: string) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newMessages, skill: 'stakeholder' }),
      });
  ...
  const handleNewChat = useCallback(() => {
    if (messages.length > 1 && !confirm('Start over? Current prep will be lost.')) {
      return;
    }
    setMessages(STAKEHOLDER_INITIAL_MESSAGES);
    setStakeholder(INITIAL_STAKEHOLDER);
    setError(null);
    setInputValue('');
  }, [messages.length]);
  ...
  const handleCopyStakeholder = useCallback(async () => {
    const markdown = generateStakeholderMarkdown(stakeholder);
    await navigator.clipboard.writeText(markdown);
    showToast('Prep copied to clipboard');
  }, [stakeholder]);

  const handleDownloadStakeholder = useCallback(() => {
    const markdown = generateStakeholderMarkdown(stakeholder);
    const blob = new Blob([markdown], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `stakeholder-prep-${new Date().toISOString().split('T')[0]}.md`;
    a.click();
  ...
  return (
    <div className="flex h-screen flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative" ref={skillDropdownRef}>
            ...
            >
              ...
              <span className="text-text-secondary">Stakeholder</span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="14"
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="border-border-light border-t p-4">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                placeholder="What decision do you need to defend?"
                disabled={isLoading}
                rows={1}
        ...
        {/* Right panel - Stakeholder Prep */}
        <div className="bg-bg-secondary w-3/5">
          <StakeholderPanel
            stakeholder={stakeholder}
            onViewStakeholder={() => setIsModalOpen(true)}
            onCopyStakeholder={handleCopyStakeholder}
            onDownloadStakeholder={handleDownloadStakeholder}
          />
        </div>
      </div>
      ...
      <StakeholderModal
        ...
        onCopy={() => showToast('Prep copied to clipboard')}
      />
