...
import { Message, UserResearch } from '@/types';
import { INITIAL_RESEARCH, RESEARCH_INITIAL_MESSAGES } from '@/lib/constants';
import {
  parseResearchUpdate,
  mergeResearchUpdate,
  generateResearchMarkdown,
} from '@/lib/researchParser';
import { ResearchModal, ResearchPanel } from '@/components/Research';
...
// Research-specific icon (users/people)
...
function ResearchModeContent() {
  return <ResearchModeInner />;
}

export default function ResearchMode() {
  return (
    <Suspense fallback={<ResearchModeLoading />}>
      <ResearchModeContent />
    </Suspense>
  );
}

function ResearchModeLoading() {
  return (
...
function ResearchModeInner() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [messages, setMessages] = useState<Message[]>(RESEARCH_INITIAL_MESSAGES);
  const [research, setResearch] = useState<UserResearch>(INITIAL_RESEARCH);
  ...
  const handleSendMessageDirect = async (content: string) => {
    ...
    const newMessages = [...RESEARCH_INITIAL_MESSAGES, userMessage];
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newMessages, mode: 'research' }),
      });
      if (!response.ok) throw new Error('Failed to get response');
      const data = await response.json();
      const { displayContent, researchUpdate, identifiedPattern } = parseResearchUpdate(
        data.message
      );
      ...
      if (researchUpdate) setResearch((prev) => mergeResearchUpdate(prev, researchUpdate));
    } catch (err) {
  ...
  const handleNewChat = useCallback(() => {
    if (messages.length > 1 && !confirm('Start over? Current research will be lost.')) return;
    setMessages(RESEARCH_INITIAL_MESSAGES);
  ...
  const handleCopyResearch = useCallback(async () => {
    ...
    showToast('Research canvas copied to clipboard');
  }, [research]);

  const handleDownloadResearch = useCallback(() => {
    const markdown = generateResearchMarkdown(research);
    const blob = new Blob([markdown], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `user-research-${new Date().toISOString().split('T')[0]}.md`;
    a.click();
  ...
  return (
    <div className="flex h-screen flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative" ref={modeDropdownRef}>
            ...
            >
              ...
              <span className="text-text-secondary">Research Mode</span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="14"
                ...
                <div className="p-2">
                  ...
                  {MODES.map((mode) => (
                    <Link
                      ...
                      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors ${mode.id === 'research' ? 'bg-accent-primary/10 text-accent-primary' : 'hover:bg-bg-secondary text-text-primary'}`}
                    >
          ...
          <div className="border-border-light border-t p-4">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                placeholder="Tell me about your product and users..."
                disabled={isLoading}
                rows={1}
      ...
      <ResearchModal
        ...
        onCopy={() => showToast('Research canvas copied to clipboard')}
      />
      <Toast
