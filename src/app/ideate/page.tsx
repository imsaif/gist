...
import { Message, Ideation } from '@/types';
import { INITIAL_IDEATION, IDEATION_INITIAL_MESSAGES } from '@/lib/constants';
import {
  parseIdeationUpdate,
  mergeIdeationUpdate,
  generateIdeationMarkdown,
} from '@/lib/ideationParser';
import { IdeationModal, IdeationPanel } from '@/components/Ideation';
...
function IdeateModeContent() {
  return <IdeateModeInner />;
}

export default function IdeateMode() {
  return (
    <Suspense fallback={<IdeateModeLoading />}>
      <IdeateModeContent />
    </Suspense>
  );
}

function IdeateModeLoading() {
  return (
...
function IdeateModeInner() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [messages, setMessages] = useState<Message[]>(IDEATION_INITIAL_MESSAGES);
  const [ideation, setIdeation] = useState<Ideation>(INITIAL_IDEATION);
  ...
  useEffect(() => {
    ...
    if (q && !initialMessageSent && !isLoading) {
      ...
      router.replace('/ideate', { scroll: false });
  ...
  const handleSendMessageDirect = async (content: string) => {
    ...
    const newMessages = [...IDEATION_INITIAL_MESSAGES, userMessage];
    ...
    try {
      ...
      const { displayContent, ideationUpdate, identifiedPattern } = parseIdeationUpdate(
        data.message
      );
      ...
      if (ideationUpdate) setIdeation((prev) => mergeIdeationUpdate(prev, ideationUpdate));
    } catch (err) {
  ...
  const handleNewChat = useCallback(() => {
    if (messages.length > 1 && !confirm('Start over? Current ideation will be lost.')) return;
    setMessages(IDEATION_INITIAL_MESSAGES);
    setIdeation(INITIAL_IDEATION);
    setError(null);
    setInputValue('');
  }, [messages.length]);
  ...
  const handleCopyIdeation = useCallback(async () => {
    const markdown = generateIdeationMarkdown(ideation);
    await navigator.clipboard.writeText(markdown);
    showToast('Options board copied to clipboard');
  }, [ideation]);

  const handleDownloadIdeation = useCallback(() => {
    const markdown = generateIdeationMarkdown(ideation);
    const blob = new Blob([markdown], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `ideation-${new Date().toISOString().split('T')[0]}.md`;
    a.click();
    ...
    <div className="flex h-screen flex-col">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="bg-bg-secondary w-3/5">
          <IdeationPanel
            ideation={ideation}
            onViewIdeation={() => setIsModalOpen(true)}
            onCopyIdeation={handleCopyIdeation}
            onDownloadIdeation={handleDownloadIdeation}
          />
        </div>
      </div>
      ...
      <IdeationModal
        ...
        onCopy={() => showToast('Options board copied to clipboard')}
      />
      <Toast
