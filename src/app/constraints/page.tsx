...
import { Message, ConstraintMap } from '@/types';
import { INITIAL_CONSTRAINTS, CONSTRAINTS_INITIAL_MESSAGES } from '@/lib/constants';
import {
  parseConstraintsUpdate,
  mergeConstraintsUpdate,
  generateConstraintsMarkdown,
} from '@/lib/constraintsParser';
import { ConstraintsModal, ConstraintsPanel } from '@/components/Constraints';
...
function ConstraintsModeContent() {
  return <ConstraintsModeInner />;
}

export default function ConstraintsMode() {
  return (
    <Suspense fallback={<ConstraintsModeLoading />}>
      <ConstraintsModeContent />
    </Suspense>
  );
}

function ConstraintsModeLoading() {
  return (
...
function ConstraintsModeInner() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [messages, setMessages] = useState<Message[]>(CONSTRAINTS_INITIAL_MESSAGES);
  const [constraintMap, setConstraintMap] = useState<ConstraintMap>(INITIAL_CONSTRAINTS);
  ...
  useEffect(() => {
    ...
    if (q && !initialMessageSent && !isLoading) {
      ...
      router.replace('/constraints', { scroll: false });
  ...
  const handleSendMessageDirect = async (content: string) => {
    ...
    const newMessages = [...CONSTRAINTS_INITIAL_MESSAGES, userMessage];
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newMessages, mode: 'constraints' }),
      });
      if (!response.ok) throw new Error('Failed to get response');
      const data = await response.json();
      const { displayContent, constraintsUpdate, identifiedPattern } = parseConstraintsUpdate(
        data.message
      );
      ...
      if (constraintsUpdate)
        setConstraintMap((prev) => mergeConstraintsUpdate(prev, constraintsUpdate));
    } catch (err) {
  ...
  const handleNewChat = useCallback(() => {
    if (messages.length > 1 && !confirm('Start over? Current constraints will be lost.')) return;
    setMessages(CONSTRAINTS_INITIAL_MESSAGES);
    setConstraintMap(INITIAL_CONSTRAINTS);
    setError(null);
    setInputValue('');
  }, [messages.length]);
  ...
  const handleCopyConstraints = useCallback(async () => {
    const markdown = generateConstraintsMarkdown(constraintMap);
    await navigator.clipboard.writeText(markdown);
    showToast('Constraint map copied to clipboard');
  }, [constraintMap]);

  const handleDownloadConstraints = useCallback(() => {
    const markdown = generateConstraintsMarkdown(constraintMap);
    const blob = new Blob([markdown], { type: 'text/markdown' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `constraints-${new Date().toISOString().split('T')[0]}.md`;
    a.click();
  ...
  return (
    <div className="flex h-screen flex-col">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative" ref={modeDropdownRef}>
            ...
            >
              ...
              <span className="text-text-secondary">Constraints Mode</span>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="14"
                height="14"
                ...
                <div className="p-2">
                  ...
                  {MODES.map((mode) => (
                    <Link
                      ...
                      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors ${mode.id === 'constraints' ? 'bg-accent-primary/10 text-accent-primary' : 'hover:bg-bg-secondary text-text-primary'}`}
                    >
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              {messages.map((message) => {
                ...
                return (
                  <div key={message.id}>
                    ...
                    >
                      <div
                        className={`max-w-[80%] rounded-2xl px-4 py-3 ${message.role === 'user' ? 'bg-msg-user-bg text-msg-user-text' : 'bg-msg-ai-bg text-text-primary'}`}
                      >
          ...
          <div className="border-border-light border-t p-4">
            ...
            <div className="flex gap-2">
              <textarea
                ...
                placeholder="What are you designing and what's the scope?"
                disabled={isLoading}
                rows={1}
        ...
        <div className="bg-bg-secondary w-1/2">
          <ConstraintsPanel
            ...
            onViewConstraints={() => setIsModalOpen(true)}
            onCopyConstraints={handleCopyConstraints}
            onDownloadConstraints={handleDownloadConstraints}
          />
        </div>
      </div>
      ...
      <ConstraintsModal
        ...
        onCopy={() => showToast('Constraint map copied to clipboard')}
      />
      <Toast
