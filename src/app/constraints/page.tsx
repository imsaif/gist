  ...
  const handleSendMessageDirect = async (content: string) => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: newMessages, skill: 'constraints' }),
      });
                ...
                <div className="p-2">
                  ...
                  {SKILLS.map((skill) => (
                    <Link
                      ...
                      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-colors ${skill.id === 'constraints' ? 'bg-accent-primary/10 text-accent-primary' : 'hover:bg-bg-secondary text-text-primary'}`}
                    >
